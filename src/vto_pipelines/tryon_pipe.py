...
class StableDiffusionTryOnePipeline(DiffusionPipeline):
    ...
    def __call__(
            ...
            final: Union[torch.FloatTensor, PIL.Image.Image], #fixed
        ...
        # fixed
        target_size = mask.shape[-2:]  # (height, width) 크기 가져오기
        final = torch.nn.functional.interpolate(final, size=target_size)
        final = torch.cat([final] * 2)
            ...
            for i, t in enumerate(timesteps):
                ...
                if cloth_latents is not None:
                    latent_model_input = torch.cat(
                        [latent_model_input, mask, masked_image_latents, pose_map.to(mask.dtype), final,
                         cloth_latents.to(mask.dtype)], dim=1)
                else:
                    latent_model_input = torch.cat(
                        [latent_model_input, mask, masked_image_latents, pose_map.to(mask.dtype), final], dim=1)
