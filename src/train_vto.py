...
def main():
    ...
    new_in_channels = 28 if args.cloth_input_type == "none" else 32
    ...
    outputlist = ['image', 'pose_map', 'captions', 'inpaint_mask', 'im_mask', 'category', 'im_name','im_parse_final']
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(inversion_adapter):
                ...
                # Image Parse
                final = batch["im_parse_final"].to(weight_dtype)
                final = torch.nn.functional.interpolate(final, size=(512 // 8, 384 // 8))
                ...
                if cloth_latents is not None:
                    unet_input = torch.cat(
                        [noisy_latents, mask, masked_image_latents, pose_map.to(weight_dtype), cloth_latents, final], dim=1)
                else:
                    unet_input = torch.cat(
                        [noisy_latents, mask, masked_image_latents, pose_map.to(weight_dtype), final], dim=1)
