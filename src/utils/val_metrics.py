...
def compute_metrics(gen_folder: str, test_order: str, dataset: str, category: str, metrics2compute: List[str],
    ...
    if category == 'all':
        if "fid_score" in metrics2compute or "all" in metrics2compute:
            ...
            a = fid.test_stats_exists(f"{dataset}_all", mode='clean')
            print(a)
            if not a:
                print("not exist")
    ...
    print("begin")
    print(f"Number of batches in gen_loader: {len(gen_loader)}")
    for idx, (gen_batch, gt_batch) in tqdm(enumerate(zip(gen_loader, gt_loader)), total=len(gt_loader)):
        ...
        # # 확인 후 출력 구문은 제거해도 됩니다.
        # if gen_images.nelement() == 0 or gt_images.nelement() == 0:
        #     raise ValueError(f"Batch {idx}에 데이터가 없습니다.")
